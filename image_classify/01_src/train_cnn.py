...
model = L.Classifier(net.ImageClassifyUsingCNN(),lossfun=F.softmax_cross_entropy)
...
for epoch in range(1,epoch_num+1):
    ...
    #for data in trainlist:
        x_batchlist = list()
        for data in trainlist[i:i+batchsize]:
            tmpdata = data[1]
            tmpdata = tmpdata.reshape(1,32,32)
            x_batchlist.append(tmpdata)

        x = np.asarray( x_batchlist )
        x = Variable(x)
...
for datatype in results:
    for evaluate_criteria in results[datatype]:
        outputfilename = "cnn_result_" + datatype + '_' + evaluate_criteria + ".txt"
